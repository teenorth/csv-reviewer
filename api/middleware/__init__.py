...
from marshmallow import exceptions as m_exceptions
import logging
...
def request_schema(schema_object):
    """Will validate that one of the roles provided are
    present in the users JWT token
    """
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                schema.load(data)
                return func(*args, **kwargs)
            except m_exceptions.ValidationError as e:
                logging.error(f"Failed to validate '{schema_object.__name__}': {e}")
                return api_response(
